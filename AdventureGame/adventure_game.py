def adventure_game():
  ...
  class ShipRoom:
    def __init__(self, name, issue, issue_broken, issue_fixed, issue_is_fixed, left, right, forward, behind):
      self.name = name
      self.issue = issue
  ...
  engine_room = ShipRoom('the engine room', 'the engines', 'The engines are silent.', 'The engines are humming away.', False, False, False, True, False)
  medbay_room = ShipRoom('the medbay', 'the bioscanner', 'The bioscanner is flashing red.', 'The bioscanner is indicating green.', False, True, True, True, True)
  weapons_room = ShipRoom('the weapons room', 'the lasers', 'The lasers are misaligned.', 'The lasers are perfectly aligned.', False, False, False, False, True)
  shields_room = ShipRoom('the shields room', 'the shields', 'The shields are flickering.', 'The engines are holding strong.', False, False, False, False, True)
  navigation_room = ShipRoom('the navigation room', 'the starmap', 'The starmap is faded.', 'The starmap is nice and vivid.', False, False, False, False, True)
  ...
  def describe_room(room):
    ...
    if (room.left):
      print('To the left is the ' + room.left.name + ".")
    if (room.right):
      print('To the right is the ' + room.right.name + ".")
    if (room.forward):
      print('Ahead of you is the ' + room.forward.name + ".")
    if (room.behind):
      print('Behind you is the ' + room.behind.name + ".")
  ...
  if (victory_conditions_met == 6):
    ...
    return
  print("The ship is repaired, and you're ready to blast off. You win!")
